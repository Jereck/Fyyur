...
def show_venue(venue_id):
  ...
  data = {
    ...
    "upcoming_shows": uc_shows,
    "past_shows_count": len(past_shows),
    "upcoming_shows_count": len(uc_shows)
  }
...
def edit_artist(artist_id):
  ...
  # If there is an artist, populate the form fields
  if artist:
...
def edit_artist_submission(artist_id):
  ...
  if error:
    flash('An error occurred and the Artist could not be changed')
  if not error:
...
def delete_artist(artist_id):
  try:
    ...
    flash('Artist ' + artist_name + ' has been removed')
  except:
    flash('An error has occured when deleting ' + artist_name + '.')
    db.session.rollback()
  finally:
    db.session.close()
